...
struct PinArea2: Decodable {
    let NIUPartialCampus: [Pin2]?
...
class SecondViewController: UIViewController, MKMapViewDelegate, CLLocationManagerDelegate {
    ...
    @IBOutlet weak var mapTypeControl2: UISegmentedControl!
    ...
    func readJson(){
        //main url
        let jsonUrlString = "http://faculty.cs.niu.edu/~krush/f18/niucampus-json"
        ...
        URLSession.shared.dataTask(with: (url!)) {(data, response, err) in
            ...
            //this will get allll the locations and give them to the annotations
            do{
                let result = try JSONDecoder().decode([PinArea2].self, from: data)
                print(result)
